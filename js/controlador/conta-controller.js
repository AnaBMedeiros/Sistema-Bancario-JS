class ContaController {
    ...
    inserir(evento) {
        ...
        const elementodataAniversario =
document.querySelector('#dataAniversario'),
        const conta = new Conta(elementoNumero.value,
            Number(elementoSaldo.value),
            elementodataAniversario.value);
      if (conta = ContaPoupanca){document.querySelector('#ContaPoupanca');} 
      else if (conta = ContaBonificada){document.querySelector('#ContaBonificada')}
      else {document.querySelector('#Conta')}
        this.repositorioContas.adicionar(conta);
        this.inserirContaNoHTML(conta);
    }
